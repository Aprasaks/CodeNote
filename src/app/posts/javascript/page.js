...
export default function JavaScriptPostsPage() {
  const postsDir = path.join(process.cwd(), "src/app/posts/javascript");
  ...
  return (
    <main className="min-h-screen bg-black text-white p-8">
      <h1 className="text-4xl font-bold mb-4">JavaScript Posts</h1>
      <p className="text-gray-400">JavaScript 관련 포스트 모음</p>

      <div className="mt-8 space-y-4">
        {posts.map((post) => (
          <Link
            key={post.slug}
            href={`/posts/javascript/${post.slug}`}
